...
parameters {
  ...
  // Sparsity prior for other
  array[N_oth] vector[N_trans_types] beta_tilde_oth;
  array[N_oth] vector<lower=0, upper=1>[N_trans_types] gamma;
  array[N_oth > 0] real<lower=0> tau1;
  array[N_oth > 0] real<lower=tau1> tau2;
...
transformed parameters {
  ...
  if(N_oth > 0){
    for(k in 1:N_oth){
      beta_oth[k] = pow(tau1[1], 1 - 0.5) * beta_tilde_oth[k];
    }
  }
...
model {
  ...
  if(N_oth > 0){
    for(k in 1:N_oth){
      for(j in 1:N_trans_types) {
        target += log_mix(
          gamma[k][j],
          normal_lpdf(beta_tilde_oth[k][j] | 0, pow(tau1[1], 0.5)), 
          normal_lpdf(beta_tilde_oth[k][j] | 0, pow(tau1[1], 0.5 - 1) * tau2[1])
        );
      }
      gamma[k] ~ beta(3, 2);
    }
