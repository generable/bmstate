...
```{r}
create_test_subject <- function(state, ages = c(20, 30, 40, 50, 60, 70, 80), dage = 20, sex = 0) {
  pdf <- NULL
  sdf <- NULL
  ldf <- NULL
  j <- 0
...
We can use it like this to predict the state occupancy probabilities for the same subject considering what if they were different age.
...
ggplot(df_test, aes(x = time, y = mean(prob), group = subject_id, color = age)) +
  facet_wrap(. ~ State) +
  geom_line() +
  scale_color_viridis_c() +
  ylab("State probability")
