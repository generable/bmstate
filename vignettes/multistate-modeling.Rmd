...
Transitioning from one state to another (or even the same one) can be called a 
*transition event*. Additionally, for example if the multistate system is modeling medical patients
as observation units, 
transitioning to a state can correspond to a *clinical event*. For now,
we assume that transitioning to state $e$ means that (clinical) 
event $e \in {1, \ldots, S}$ occurs.
We often ultimately want the model to be able to predict, for a given 
observation unit, the probability that a particular (clinical) event $e$
