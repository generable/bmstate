...
competing, meaning that occurrence of one censors the remaining ones.
...
a continuous-time Markov model. In this special case we can utilize
the Kolmogorov forward equations.

Let \( P_{ij}(t) \) be the probability that the process is in state \( j \) at time \( t \), given that it started in state \( i \) at time 0. The Kolmogorov forward equations (also known as the forward differential equations) are given by:

\[
\frac{d}{dt} P_{ij}(t) = \sum_{k} P_{ik}(t) q_{kj}, \quad \text{for all } i, j
\]
