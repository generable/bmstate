...
params <- list(
  ...
  hazard_only = TRUE,
  do_cf = FALSE,
  sm_name = "msm-general-mixp.stan"
)

NP <- params$iter_sampling # number of paths to sample per sub when gen paths
NR <- 2
all_states <- c("Randomization", "Bleed", "Fatality", "Censor")
terminal_states <- as.character(na.omit(stringr::str_extract(all_states, ".*Fatality.*")))

source(here::here("exploratory_multistate/R/sim.R"))
source(here::here("exploratory_multistate/R/draws.R"))
source(here::here("exploratory_multistate/R/PathData.R"))
source(here::here("exploratory_multistate/R/pathgen.R"))
source(here::here("exploratory_multistate/R/plot.R"))
source(here::here("exploratory_multistate/R/standata.R"))
source(here::here("exploratory_multistate/R/summarize.R"))
source(here::here("exploratory_multistate/R/scores.R"))
...
stan_file <- here::here(
  paste0("exploratory_multistate/stan/", params$sm_name)
)
...
run_experiment <- function(N_subject, h0_true, df_beta_true) {
  # Simulate data
  d <- simulate_multitransition_data(
    N_subject, possible_covs, h0_true, df_beta_true, 2
  )
  pd <- d$pd
  ...
  stan_dat <- create_stan_data(
    pd, NULL, params$covariates, "age", "age", "age",
  ...
  m_sub <- d$m_sub
  out <- dplyr::lst(
    fit, stan_dat, pd, dt, paths_3yr, paths_3yr_oos,
...
for (baz in H0) {
  ...
  baz[2] <- 2 * baz[2]
  baz[3] <- 1.5 * baz[3]
  r0 <- run_experiment(params$N_subject, baz, df_beta_true0)
  p0 <- run_experiment_postproc(r0)
  r1 <- run_experiment(params$N_subject, baz, df_beta_true1)
