...
hazard_mult_recovery <- function(res) {
  ...
  df <- tibble::as_tibble(df |> left_join(res$pd$subject_df, by = "subject_id"))
  df <- df |> left_join(dt$legend, by = "transition")
...
plot_inst_haz_recovery <- function(res, trans_idx) {
  ...
  b <- df_ih |>
    ...
    dplyr::slice(1) |>
    dplyr::select(-inst_haz)
  a <- df_ih |>
    dplyr::group_by(subject_id) |>
    dplyr::summarize(ih_mean = mean(inst_haz))
