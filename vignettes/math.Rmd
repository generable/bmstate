...
```{r, fig.width=6}
set.seed(383)
mod <- create_msm(tmat, n_grid = 100)
mod$set_prior_mean_h0(c(0.1, 0.01, 0.05))
dat <- mod$simulate_data(N_subject = 1)
...
The package relies on numerical integration of the hazard functions. This is
required to evaluate the likelihood, which involves integrating over the
intervals that were at risk.
