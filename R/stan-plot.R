...
plot_stan_data_integral <- function(model, stan_data, iidx, h0 = 0.001, w = NULL) {
  mod <- model
  sd <- stan_data
  checkmate::assert_number(h0, lower = 0)
  checkmate::assert_integerish(iidx, lower = 1, upper = sd$N_int)

  if (is.null(w)) {
    w <- stats::rnorm(mod$system$num_weights(), sd = 0.5)
  }
  ...
  ts <- paste0(
    "interval_idx = ", iidx, ", correction_multiplier = ",
    round(sd$correction_multiplier[iidx], 5)
  )
  ggplot(data.frame(time = tt, hazard = ll), aes(x = .data$time, y = .data$hazard)) +
    stat_function(
      fun = hfun,
      xlim = xint,
      geom = "area",
    ...
    geom_col(
      ...
      fill = "firebrick3", col = "firebrick3", width = sd$delta_grid, alpha = 0.3
    ) +
    geom_line(lwd = 1) +
...
#' @param subject_idx Index of subject whose data rows to plot.
#' @family Stan-related functions
plot_stan_data_matrix <- function(model, stan_data, name, subject_idx) {
...
plot_intervals_mat <- function(model, sd, name, subject_idx) {
  cols <- which(sd$idx_sub == subject_idx)
  if (length(cols) == 0) {
